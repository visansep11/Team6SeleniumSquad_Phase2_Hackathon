...
public class AssignStaffPage {
	...
	public void validateWithoutProgram() throws InterruptedException {
		...
		/*
		 * Thread.sleep(5000); wait = new WebDriverWait(driver, Duration.ofSeconds(30));
		 * wait.until(ExpectedConditions.visibilityOfElementLocated(selectBatchBox));
		 * String desiredBatch = "squad batch 60"; WebElement selectedBatch = null; for
		 * (WebElement option : driver.findElements(selectBatchBox)) { if
		 * (option.getText().trim().equals(desiredBatch)) { selectedBatch = option;
		 * break; } } if (selectedBatch != null) { ((JavascriptExecutor)
		 * driver).executeScript("arguments[0].click();", selectedBatch);
